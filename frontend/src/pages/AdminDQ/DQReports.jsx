...
export default function DQReports() {
  ...
  function dlSheet(ext) {
    const s = prompt("Enter DQ sheet name:");
    if (!s) return;
    const norm = s.trim().toUpperCase().replace(/\s+/g, "_");
    ...
    <div style={{ padding: "20px", minHeight: "100vh", background: "#f8fafc" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          ...
          <button onClick={() => dlSheet("pdf")}   style={btnPrimary}>⬇ PDF</button>
        </div>
      </div>
      ...
      {data && (
        <>
          ...
          <SheetProgressBars rows={allRows} assignedKey="totalAssigned" completedKey="allTimeCompleted" />
        </>
      )}
...
function SheetProgressBars({ rows, assignedKey = "totalAssigned", completedKey = "allTimeCompleted" }) {
  const sheetMap = {};
  rows.forEach(row => {
